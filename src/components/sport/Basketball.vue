<template>
	<ul class="sport__championship-list">
		<ChampionshipCard
			v-for="(item, index) in basketballTournaments"
			:key="index"
			:item="item"
			:league="league"
		/>
...
export default {
	...
	computed: {
		...mapGetters([
			'GET_BASKETBALL_TOURNAMENTS',
		]),
		matchesApi() {
			return new MatchesApi()
		},
		basketballTournaments() {
			if (this.GET_BASKETBALL_TOURNAMENTS.items) {
				if (this.league === 'FANTASY') {
					return this.GET_BASKETBALL_TOURNAMENTS.items.filter((item) => item?.fantasy === true)
				} else {
					return this.GET_BASKETBALL_TOURNAMENTS.items.filter((item) => item?.fantasy === false)
				}
			} else {
				return []
			}
